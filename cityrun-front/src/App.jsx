...
function App() {
  ...
  const handleRecommend = async () => {
    ...
    // 🔻🔻🔻 1. CATCH 블록 수정 🔻🔻🔻
      ...
      // API 상태 텍스트에는 개발용 상세 오류 메시지를 그대로 표시
      const rawErrorMessage = err.response?.data?.message || 
                              err.response?.data?.error || // 💡 혹시 'error' 필드로 올 경우 대비
                             '경로 추천 요청 중 오류가 발생했습니다.';
      setError(rawErrorMessage); 

      // 400 에러 (이상치, 경로 없음 등)일 경우
      if (err.response?.status === 400) {
        const userMessage = "경로를 찾을 수 없습니다. 출발지를 다시 설정해주세요.";
        alert(userMessage); // ⬅️ 2. 사용자에겐 alert() 경고창 표시
        speak(userMessage); // ⬅️ 3. TTS도 alert과 동일하게 안내
      } else {
        // 500 에러 등 그 외 서버 오류
        const userMessage = "경로를 찾을 수 없습니다. 출발지를 다시 설정해주세요.";
        alert(userMessage);
  ...
  return (
    <div style={styles.container}>
      ...
      <p style={styles.notice}>
        ...
        시내에서 선택해주세요. (서울역 12번 출구 - 알고리즘 테스트 최적화 데이터)
      </p>
